...
Preferred agent pattern: `preflight_dalfox` → `scan_with_dalfox` → poll `get_results_dalfox` (respect `suggested_poll_interval_ms`) → `delete_scan_dalfox` when terminal and settled.
...
4. Loop: `get_results_dalfox` (respect interval) until terminal status and
   `settled: true`.
5. Present findings (lead with V, then A, then R — see `results.md`).
6. `delete_scan_dalfox` (optional — call only when `settled: true`; if
   cancellation just happened, retry after the worker-draining interval;
   terminal jobs auto-expire).
