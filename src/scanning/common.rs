...
use crate::payload::{XSS_DOM_PAYLOADS, XSS_HTML_PAYLOADS, XSS_JAVASCRIPT_PAYLOADS, XSS_PAYLOADS};

pub fn get_xss_payloads() -> &'static [&'static str] {
    XSS_PAYLOADS
}

pub fn get_reflection_payloads(args: &ScanArgs) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    if args.only_custom_payload {
        ...
        base_payloads.extend(XSS_JAVASCRIPT_PAYLOADS.iter().map(|s| s.to_string()));
        base_payloads.extend(XSS_HTML_PAYLOADS.iter().map(|s| s.to_string()));
        if !args.fast_scan {
            if let Some(path) = &args.custom_payload {
                base_payloads.extend(load_custom_payloads(path)?);
            }
...
pub fn get_dom_payloads(args: &ScanArgs) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut base_payloads = vec![];
    ...
    if args.only_custom_payload {
        ...
        base_payloads.extend(XSS_DOM_PAYLOADS.iter().map(|s| s.to_string()));
        if !args.fast_scan {
            if let Some(path) = &args.custom_payload {
                base_payloads.extend(load_custom_payloads(path)?);
            }
