...
fn test_sarif_schema_compliance() {
    ...
    let json: Value = serde_json::from_str(&sarif)
    ...
    assert!(driver["informationUri"].is_string(), "driver.informationUri must be present");
    assert!(driver["version"].is_string(), "driver.version must be present");
    ...
    assert!(rule["shortDescription"].is_object(), "rule.shortDescription must be an object");
    assert!(rule["shortDescription"]["text"].is_string(), "rule.shortDescription.text must be a string");
    assert!(rule["fullDescription"].is_object(), "rule.fullDescription must be an object");
    ...
    assert!(result["ruleId"].is_string(), "result.ruleId must be a string");
    assert!(result["level"].is_string(), "result.level must be a string");
    assert!(result["message"].is_object(), "result.message must be an object");
    assert!(result["message"]["text"].is_string(), "result.message.text must be a string");
    assert!(result["locations"].is_array(), "result.locations must be an array");
    ...
    assert!(location["physicalLocation"].is_object(), "location.physicalLocation must be an object");
    
    let physical_location = &location["physicalLocation"];
    assert!(physical_location["artifactLocation"].is_object(), "artifactLocation must be an object");
    assert!(physical_location["artifactLocation"]["uri"].is_string(), "uri must be a string");
    
    // Validate properties
    assert!(result["properties"].is_object(), "result.properties must be an object");
    let props = result["properties"].as_object().unwrap();
    
    // Verify custom properties are present
    assert!(props.contains_key("type"), "properties should contain 'type'");
    assert!(props.contains_key("inject_type"), "properties should contain 'inject_type'");
    assert!(props.contains_key("method"), "properties should contain 'method'");
    assert!(props.contains_key("param"), "properties should contain 'param'");
    assert!(props.contains_key("payload"), "properties should contain 'payload'");
    assert!(props.contains_key("severity"), "properties should contain 'severity'");
    
    // When include_request and include_response are true
    assert!(props.contains_key("request"), "properties should contain 'request'");
    assert!(props.contains_key("response"), "properties should contain 'response'");
}
...
fn test_sarif_message_with_evidence() {
    ...
    assert!(message.contains("XSS detected"), "Message should contain main text");
    assert!(message.contains("Evidence: Found unescaped output"), "Message should contain evidence");
}
...
fn test_sarif_partial_fingerprints() {
    ...
    assert!(fingerprints.is_object(), "partialFingerprints should be an object");
    assert_eq!(fingerprints["messageId"], "12345", "messageId should be included");
}
...
fn test_sarif_rule_metadata() {
    ...
    assert!(rule["shortDescription"]["text"].as_str().unwrap().contains("XSS"));
