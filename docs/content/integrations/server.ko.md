...
IP 리터럴과 `localhost`는 항상 허용되고, 그 외 호스트명은 명시해야 합니다:
...
`error` / `cancelled` 스캔의 부분 탐지 결과입니다. 대상에 끝내 닿지 못했거나 시작 전에
취소된 스캔은 테스트한 것이 없으므로 `results` 자체가 없습니다. 실행 중에 취소한 스캔은 곧바로
...
스캔이 조용히 돌아가는 것보다 낫기 때문입니다. `remote_payloads`와 `remote_wordlists`도
같은 방식으로 검사합니다. 등록되지 않은 프로바이더 이름은 아무것도 받아오지 못한 채,
요청한 페이로드 커버리지가 빠진 스캔을 `done`으로 보고하게 만들기 때문입니다.

`blind`는 비어 있거나(= 블라인드 XSS 사용 안 함) `http://` / `https://`로 시작해야
합니다. 이 값을 설정하면 *저장형* 블라인드 XSS 주입이 켜집니다 — `<script src=...>`
페이로드가 모든 쿼리·바디·헤더·쿠키 파라미터에 기록되어 대상에 그대로 남습니다.
그래서 콜백을 받을 수 없는 값은 그 페이로드만 남기고 아무 소득도 없으므로 `400`으로
거부합니다.
