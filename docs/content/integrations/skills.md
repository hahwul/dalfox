...
- **CLI scenarios:** POST bodies, authenticated sessions, proxy-through-Burp, blind XSS (your own callback URL, or a Dalfox-managed OAST session with `--blind-oob`), stored XSS, target lists, captured requests (raw-http / HAR), fast smoke tests, and maximum-coverage runs.
- **Result interpretation:** the three separate axes — `type` (the claim: `V` asserted vulnerable > `A` AST-detected > `R` reflected-only > `I` informational), `detection_method` (how it was found), and `severity` (impact, today derived from the tier) — plus the `confidence` / `confidence_reason` grade behind the claim. The agent leads with the strongest claims and never describes `V` as observed browser execution. See [Detection Model](../../guide/detection-model/).
