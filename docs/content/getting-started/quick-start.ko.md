...
| 태그 | 의미 |
|-----|---------|
| `[V]` | **검증됨(Verified)**: 파싱된 응답에서 페이로드가 실제 DOM 요소로 확인됨(Dalfox 마커에 대한 CSS 셀렉터 매칭) |
...
`[V]`는 브라우저 실행이 아닙니다. Dalfox에는 헤드리스 브라우저가 없어서, 순수 클라이언트 사이드 DOM-XSS는 `[A]`가 최상위 등급이며 브라우저에서 직접 확인해야 합니다. 각 서브시스템이 실제로 무엇을 증명하는지는 [탐지 모델](../../guide/detection-model/) 문서를 참고하세요.
