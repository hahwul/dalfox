...
| `--state-file` | — | — | Record completed targets to a file and skip them when the scan is re-run; raw HTTP/HAR request data is fingerprinted so changed captures are scanned again; credential values (cookies, `Authorization`, token-style headers) are excluded so a refreshed session resumes |
