...
A target's resume identity is its URL, its method, and a hash of the request data it is sent with: body, header names and values, cookie names, and user-agent. That covers data captured in raw HTTP and HAR inputs as well as `-H`, `--cookies`, and `--user-agent`, so a changed capture is scanned again. **Credential values are left out**: the values of `Cookie`, `Authorization`, `Proxy-Authorization`, and headers whose name contains `token`, `auth`, `session`, `secret`, `csrf`/`xsrf`, `api-key`, or `signature`, and every cookie value. Refreshing a session before resuming is the normal re-auth step and tests the same requests, so it resumes instead of rescanning; adding or removing a header or cookie still counts. The flip side: switching to a different *account* with the same cookie names also resumes — use a separate `--state-file` per identity. Only the hash is stored; the values never reach the state file. Invocations with the same flags produce the same identity, so one state file can back a shell loop of per-URL invocations as easily as a single `--input-type file` run.
