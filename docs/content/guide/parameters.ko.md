...
6. **DOM verification(DOM 검증):** 응답을 파싱하여 페이로드가 실행 가능한 위치에 도달했는지 확인합니다. AST 기반 DOM-XSS 분석은 파라미터당 한 번, 빠른 프로브의 응답으로 실행되며, 빠른 프로브를 건너뛴 경우에는 첫 반사 응답으로 실행됩니다.
...
사전과 DOM 후보는 이름마다 요청을 하나씩 보내지 않고, 제한된 쿼리 버킷으로
검증합니다. 기본 버킷은 최대 64개의 canary를 담고 요청 라인을 약 8 KiB
이하로 유지합니다. 반사된 canary는 한 번의 응답에서 자신의 파라미터 이름을
식별합니다. 버킷의 응답은 달라졌지만 canary가 반사되지 않으면 같은 폭의
control 요청을 보내고, 양성 버킷만 4개로 나눠 metric-only 파라미터를 찾습니다.
따라서 큰 워드리스트도 넓게 확인하면서 모호한 경우에만 추가 요청을 사용합니다.

판정은 이름 단위로 유지됩니다. metric-only 결과는 동일한 깨끗한 요청 두 번이
같은 응답을 돌려줄 때만 신뢰하고, 요청마다 본문이 바뀌는 페이지(순환 위젯,
렌더링 시간 표시 등)에서는 상태 코드 변화만 인정하며, 이름 하나를 채택하기
전에 한 번 더 보내 확인합니다. 다른 이름이 반사된 버킷에 남은 이름은 반사된
이름을 빼고 다시 검사합니다. 요청이 실패했거나 서버가 control과 똑같이 거부한
버킷(예: 쿼리 길이 제한)은 버리지 않고 나눠서 다시 보내며, 하나의 리다이렉트
`Location`에 함께 나타난 이름들은 모두 리다이렉트 파라미터로 보지 않고 더 작은
그룹으로 나눠 확인합니다.
...
마커를 지정하면 탐색과 마이닝을 건너뜁니다. `FUZZ`를 포함한 쿼리 값, form 본문 값, 최상위 JSON 문자열 값, 헤더 값, 쿠키 값이 각각 파라미터가 되고, 각 페이로드는 그 값 전체를 대체합니다. 그 밖의 위치(경로 세그먼트, 중첩된 JSON 필드)에 둔 마커는 인식하지 않습니다.
...
같은 게이트가 무해한 형태 몇 가지를 더 걸러냅니다. `<script>` 안에만 반사되었고 파싱한 자바스크립트상 싱크 호출이 생기지 않는 경우, 그리고 URL 값 속성 밖에서 서버가 이스케이프(퍼센트 또는 엔티티 인코딩)해서 되돌려준 경우입니다.
