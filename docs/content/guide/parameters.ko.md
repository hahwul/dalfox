...
3. **Active probing(능동 프로빙):** 각 파라미터에 프로브를 하나씩 보내 어떤 특수 문자가 살아남는지 파악합니다.
...
7. **DOM verification(DOM 검증):** 응답을 파싱하여 페이로드가 실제 요소를 형성하는지 확인합니다. AST 기반 DOM-XSS 분석은 빠른 프로브에서 받아둔 응답으로 병렬 실행됩니다.
...
일부 엔드포인트는 페이로드가 특정 위치(예: JWT 내부)에 있어야 합니다. 이럴 때는 `--inject-marker`를 쓰세요:
