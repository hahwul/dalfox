...
대상의 식별자는 URL, 메서드, 그리고 요청에 실리는 데이터(본문, 헤더 이름과 값, 쿠키 이름, user-agent)의 해시입니다. raw HTTP·HAR 입력에 캡처된 값뿐 아니라 `-H`, `--cookies`, `--user-agent`로 준 값도 포함되므로, 캡처가 바뀌면 다시 스캔합니다. **자격 증명 값은 제외됩니다**: `Cookie`, `Authorization`, `Proxy-Authorization`, 그리고 이름에 `token`, `auth`, `session`, `secret`, `csrf`/`xsrf`, `api-key`, `signature`가 들어간 헤더의 값과 모든 쿠키 값입니다. 이어가기 전에 세션을 갱신하는 것은 일반적인 재인증 절차이고 같은 요청을 검사하므로, 다시 스캔하지 않고 이어서 진행합니다. 헤더나 쿠키를 추가·삭제하면 여전히 다시 스캔합니다. 반대로 쿠키 이름이 같은 다른 *계정*으로 바꿔도 이어서 진행되므로, 계정마다 별도의 `--state-file`을 쓰세요. 파일에는 해시만 저장되고 값 자체는 기록되지 않습니다. 같은 플래그로 실행하면 식별자도 같으므로, 하나의 state 파일로 `--input-type file` 한 번짜리 실행뿐 아니라 URL 하나씩 도는 셸 루프도 그대로 커버할 수 있습니다.
...
출력·속도 관련 플래그는 의도적으로 이 해시에서 빠져 있습니다 — `--format`, `--output`, `--silence`, `--only-poc`, `--baseline`, `--timeout`, `--scan-timeout`, `--delay`, `--rate-limit`, `--workers`, `--max-concurrent-targets`, 그리고 대상 목록 자체입니다. 중단된 스캔을 이어가면서 타임아웃을 늘리거나 속도를 낮추는 것은 자연스러운 대응이고, 이미 완료된 대상이 무엇으로 검사됐는지는 그것들로 바뀌지 않기 때문입니다. 반대로 페이로드·탐색·커버리지·인증을 바꾸는 것은 파일을 무효화합니다 — `--deep-scan`, `--encoders`, `--custom-payload`, 마이닝/탐색 토글, WAF 옵션, `--limit`, `--headers` / `--cookies`의 헤더·쿠키 *이름*(과 자격 증명이 아닌 헤더 값) 등이 여기에 해당합니다. 자격 증명 값과 `--cookie-from-raw` 경로는 위에서 설명한 대로 해시에 들어가지 않습니다.
