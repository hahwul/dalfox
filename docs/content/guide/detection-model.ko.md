...
- **중복 `R` 정리** — 같은 대상에서 같은 `(param, inject_type)`에 `V`가 있으면
...
(AST 중복은 스트림에서도 합쳐집니다). 따라서 스트림에 보였던 `R`이 최종 보고서에는 없을 수 있습니다. 둘이 다르면 최종
보고서가 정답입니다.
...
정리를 적용한 뒤 기준), `2`는 하드 에러(잘못된 입력, 모든 대상 도달 불가,
`--output` 기록 실패)이며, 결과 없이 정상적으로 끝나지 못한 실행도 포함합니다.
기본값 `--on-session-loss abort`에서 세션이 끊긴 경우나 심각한 전송 손실이
여기에 해당합니다. `R` 하나나 `--detect-outdated-libs`가 만든 `I` 하나도
