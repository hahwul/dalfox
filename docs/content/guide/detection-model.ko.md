...
Dalfox가 실제 실행을 관측하는 방식은 **대역외 콜백**(blind XSS) 하나뿐입니다. 주입된 `<script src=…>`가 콜백을 보내오면, 실제 브라우저가 그것을 파싱하고 가져간 것입니다. 이건 경험적 증거이며 Dalfox가 만들어내는 가장 강한 증거입니다. 다만 Dalfox가 제어하는 브라우저가 아니라 다른 누군가의 브라우저에서 옵니다.
...
`--stream-findings`는 결과가 기록되는 즉시 내보내는데, 이는 정리 *이전*입니다.
