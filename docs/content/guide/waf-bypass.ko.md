...
WAF 탐지와 무관하게 **모든** 워커와 대상에 걸쳐 공유되는 확고한 요청 속도 상한을 원한다면 `--rate-limit`(초당 요청 수)과 결합하세요. `--delay`는 단일 워커의 간격만 벌리므로, 공유 IP 뒤에서 스캔하거나 전역 임계값을 가진 엣지 WAF를 상대할 때 바로 이 옵션이 적합합니다:
