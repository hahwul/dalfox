...
모든 파라미터는 같은 페이로드를 받고 저장형 싱크는 받은 값을 보관하므로, 한 필드가 페이로드를 저장하고 나면 조회 페이지는 스캔 내내 그 값을 보여 줍니다. 탐지 결과를 올바른 필드에 붙이기 위해, Dalfox는 각 파라미터가 주입을 시작하기 전에 조회 페이지를 한 번 스냅샷합니다. 페이로드는 그 파라미터의 주입으로 스냅샷보다 더 많이 나타날 때에만 해당 파라미터에 귀속됩니다. 다른 필드가 먼저 저장한 사본은 이미 스냅샷에 있으므로 잘못 귀속되지 않습니다.
