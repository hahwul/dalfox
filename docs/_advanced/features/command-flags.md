...
| `-p, --param strings` | Specify parameter names to focus testing on. Dalfox will prioritize these names if found during its discovery phase (across URL queries, form bodies, JSON bodies etc.). This flag specifies only the *name* of the parameter, not its type (e.g., `-p 'username'`, not `-p 'BODY_JSON:username'`). For more precise targeting of typed parameters, especially those not easily discoverable by name alone (like specific headers or cookies without distinct names in other contexts), use the `--magic` flag or ensure the parameter is discoverable via other means.<br>Example: `-p 'username' -p 'id'` |
...
| `--magic string` | Forces XSS testing on any parameter whose value contains the specified magic string. Dalfox searches for the magic string in URL query parameters, headers, cookies, form-urlencoded body parameters, and top-level JSON body string values. When found, Dalfox bypasses normal discovery for that specific parameter and tests it using its identified type (e.g., QUERY, HEADER, COOKIE, BODY_FORM, BODY_JSON). This is useful for targeting specific injection points or re-testing findings. For more details on parameter types, see [Parameter Types](./parameter_types.md).<br>Example: `dalfox url "https://api.example.com/user" -X POST -H "Content-Type: application/json" -d '{"id":"safe","comment":"MAGIC_STRING_TEST"}' --magic="MAGIC_STRING_TEST"` |
