...
5. Clean up: `delete_scan_dalfox` (MCP) or just let the process end (CLI). For MCP, delete only after the status reports `settled: true`; after cancellation, retry once the worker has drained. Terminal jobs auto-expire after 1 h.
